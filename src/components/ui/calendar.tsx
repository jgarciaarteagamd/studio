  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        head_row: "flex",
        head_cell:
          "text-muted-foreground rounded-md flex-1 min-w-0 font-normal text-sm p-0 text-center", // Changed: w-9 to flex-1 min-w-0
        row: "flex w-full mt-2",
        cell: "h-9 flex-1 min-w-0 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20", // Changed: w-9 to flex-1 min-w-0
        day: cn(
          buttonVariants({ variant: "ghost" }),
          "h-full w-full p-0 font-normal aria-selected:opacity-100" // Changed: h-9 w-9 to h-full w-full
        ),
